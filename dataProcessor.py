...
import os
from rembert.rembert_tokenizer import RemBertTokenizer
import csv
vocab_path = '/'.join(os.path.abspath(__file__).split('/')[:-1])
tokenization = RemBertTokenizer(vocab_path + '/data/sentencepiece.model')
...
class XNLIProcessor(object):
  ...
  def get_train_examples(self, data_dir):
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "multinli.train.en.tsv")), "train")
